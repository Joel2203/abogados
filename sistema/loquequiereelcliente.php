...
<?php
  ...
  // Puedes definir valores iniciales en la sesión si es necesario
  if (!isset($_SESSION['rol'])) {
      $_SESSION['rol'] = 0; // Valor predeterminado o cualquier otro valor según tus necesidades
  }
...
.custom-button {
        padding: 10px 20px; /* Ajusta el tamaño según tus necesidades */
...
.sidebar {
  ...
  position: fixed;
  top: 0;
  right: -300px; /* Inicia el sidebar fuera de la pantalla a la derecha */
  bottom: 0;
  background: #f1f1f1;
  overflow-x: hidden;
  transition: right 0.5s ease; /* Añadida la transición para la propiedad 'right' */
  padding: 20px;
  z-index: 1; /* Asegura que el sidebar esté detrás del contenido principal */
}

.sidebar.open {
  right: 0; /* Sidebar se mueve a la posición abierta */
}
...
.toggle-btn {
  ...
  border-radius: 5px;
  padding: 5px 10px;
  z-index: 2; /* Asegura que el botón esté encima del sidebar */
}
...
<div id="sidebar" class="sidebar">
    ...
    <div class="container mt-6">
      <div class="table-responsive">
        ...
        <div class="search-container">
          ...
          <button class="custom-button" id="filterBtn" onclick="openModal()">Filtro avanzado</button>
 
          <!-- Modal para el filtro avanzado -->
          <div id="filterModal" class="modal">
            <div class="modal-content">
              <label for="columnSelect">Seleccione la columna:</label>
              <select id="columnSelect">
                <option value="0">N°</option>
                <option value="1">Sumilla(Descripción)</option>
                <option value="3">Norma de aprobación</option>
                <option value="4">Denominación</option>
                <option value="5">Fecha de publicación</option>
                <option value="6">Código de Norma</option>
                <option value="7">Ruta</option>
 
            
                <option value="8">Link</option>
   
                <option value="9">Ver</option>
          ...
          <table class="mi-tabla">
            <thead>
              <tr> 
                <th class="sortable" onclick="sortTable(0)">N°</th>
                <th class="sortable" onclick="sortTable(1)">Sumilla(Descripción)</th>
                <th class="sortable" onclick="sortTable(3)">Norma de aprobación</th>
                <th class="sortable" onclick="sortTable(4)">Denominación</th>
                <th class="sortable" onclick="sortTable(5)">Fecha de publicación</th>
                <th class="sortable" onclick="sortTable(6)">Código de Norma</th>
                <th class="sortable" onclick="sortTable(7)">Ruta</th>
                 
                  <?php if ($_SESSION['rol'] == 1) { ?>
                    <th class="sortable" onclick="sortTable(8)">Link</th>
                  <?php } ?>
               
                <th class="sortable" onclick="sortTable(9)">Ver</th>
              </tr>
            </thead>
    ...
    <?php
    // Incluir tus otras lógicas PHP

    // Imprimir el valor de $_SESSION['rol'] en una variable JavaScript
    echo '<script>var userRole = ' . json_encode($_SESSION['rol']) . ';</script>';
    ?>
...
$(document).ready(function() {
  ...
  function obtenerValoresSeleccionados() {
    ...
    // Devolver un array de texto solo
...
function obtenerValoresSeleccionados(id) {
  ...
  const fechaInicio = document.getElementById('desde').value;
  const fechaFin = document.getElementById('hasta').value;
  ...
  var dispositivoLegalSeleccionado = obtenerValoresSeleccionados();
  ...
  filtros: {
    ...
    dispositivoLegal:  dispositivoLegalSeleccionado, // Asumiendo que se llenará después con valores apropiados
    tomo: { id: "" , nombre: "" },
    materia: { id: "" , nombre: ""  },
...
function obtenerValoresSeleccionados() {
    var valoresSeleccionados = $('.select2-selection__choice').map(function() {
